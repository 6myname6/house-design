...
| `username` | VARCHAR(64) | NOT NULL，**UNIQUE** | 用户名；手机验证码自动注册的用户 username=手机号 |
| `phone` | VARCHAR(11) | **UNIQUE**，允许 NULL | 手机号（A-10 新增）；存量账号为 NULL，MySQL 唯一索引允许多个 NULL 共存 |
| `password` | VARCHAR(255) | 允许 NULL | BCrypt 加密存储；手机验证码自动注册、尚未设置密码的用户为 NULL |
| `nickname` | VARCHAR(64) | | 昵称，默认为 username |
| `avatar` | VARCHAR(255) | | 头像 URL |
| `created_at` | DATETIME | NOT NULL | 创建时间（不可更新） |

> **A-10 变更（2026-09-21）**：新增 `phone` 列与唯一索引 `uk_phone`，`password` 放宽为可空。建表语句在 `db/house_design.sql` 同步修改；存量库执行：
>
> ```sql
> ALTER TABLE `t_user`
>     ADD COLUMN `phone` VARCHAR(11) NULL COMMENT '手机号' AFTER `username`,
>     ADD UNIQUE KEY `uk_phone` (`phone`),
>     MODIFY COLUMN `password` VARCHAR(255) NULL COMMENT '密码（BCrypt，短信注册用户可空）';
> ```
...
| `scene_config` | LONGTEXT | | 3D 场景 JSON（当前为 `{"type":"photo-tour"}`，前端按 type 选择渲染模式） |
...
## Redis 键设计（登录限流 ✅ / 短信验证码 ✅ 已实现 / 登出黑名单 ⬜ 规划中，对应需求 S-3、A-10、A-9 / S-4）

> 实现：`common/LoginRateLimiter.java`（注入 `StringRedisTemplate`），由 `LoginServiceImpl.login` 在锁定检查 / 失败记录 / 成功清零三个时机调用；阈值与锁定时长配置项 `app.login-limit.max-fail-count` / `app.login-limit.lock-seconds`。
> 短信验证码（✅ 已实现，A-10）：由 `common/SmsCodeService`（同模式注入 `StringRedisTemplate`）在发送 / 校验两个时机操作双 key，契约见接口文档 §2.6/§2.7；TTL 配置项 `app.sms.code-ttl-seconds` / `app.sms.resend-interval-seconds`。
> 登出黑名单（规划中）：`common/TokenBlacklistService.java` 承载，由 `POST /api/auth/logout` 写入、`JwtInterceptor` 每次受保护请求读取，契约见接口文档 §2.5。

> 登录失败计数、短信验证码与 token 黑名单均为高频短时数据，用 Redis 而非 MySQL：`INCR`/`EXPIRE`、`SET` + TTL 自动过期，无需定时清理；业务数据仍全部落 MySQL。

| Key | 类型 | Value | TTL | 读 / 写时机 |
| --- | --- | --- | --- | --- |
| `login:fail:{username}` | String（整数） | 连续失败次数 | 600s（首次失败时设置，固定窗口不续期） | 登录前 `GET` 判断是否 ≥ 5（锁定则再 `TTL` 取剩余秒数）；失败 `INCR`（返回值=1 时 `EXPIRE`）；成功 `DEL` |
| `sms:code:{phone}` | String | 6 位数字验证码 | 300s（每次发送覆盖并重置） | 发送时 `SET`（带 TTL）；验证码登录 `GET` 比对，错误保留、成功立即 `DEL`（一次性） |
| `sms:limit:{phone}` | String | 固定 `"1"` | 60s | 发送前 `GET` 做频控（存在即拒绝，剩余等待取 `TTL`）；发送成功后 `SET`（带 TTL） |
| `jwt:blacklist:{sha256(token)}` ⬜ | String | 固定 `"1"` | token 剩余有效期（`exp - now`，最长 7 天） | 退出登录 `SET` 写入（一次性，不更新）；拦截器对每个受保护请求 `EXISTS` 校验，命中返回 401；TTL 到期 key 自动删除 |

设计说明：

1. **不建 MySQL 表**：计数 10 分钟后自动失效，落库需要定时任务清理且写压力大，Redis 天然适配。
2. **固定窗口**：只在第 1 次失败时设置 TTL，第 2～5 次失败不续期，锁定时长可预期。
3. **计数不持久化**：Redis 重启后计数清零、账号自动解锁（简历项目可接受的权衡，接口文档 §9 已注明）；生产可开 AOF/RDB 或改用持久化限流器。
4. **后续扩展**：注册防刷（A-7）新增 `register:fail:{ip}`，同模式或升级滑动窗口（ZSET）。
5. **短信双 key 分工（⬜ 开发中，A-10）**：验证码本体与频控标记拆成两个 key——`sms:code` 管"验证码 5 分钟生命周期"，`sms:limit` 管"60 秒发送间隔"，TTL 独立互不影响（若只用一个 key，无法表达"验证码还有效但已允许重发"）。
6. **验证码一次性**：校验通过立即 `DEL sms:code:{phone}`，防止同一码被重放；但**校验失败不删**，避免用户输错一位就必须重新获取。后续可加 `sms:try:{phone}` 错误尝试计数（如 5 次作废）与每日发送总量 key（接真实短信前必做）。
7. **黑名单存 token 哈希而非原文（⬜ 规划中）**：key 中放 `SHA-256(token)` 十六进制串——JWT payload 含 userId/username，避免令牌明文落 Redis；SHA-256 定长 64 位十六进制字符，key 体积可控且不可逆推。校验时对请求 token 现场哈希后 `EXISTS`。
8. **TTL 精确对齐 token 过期时间**：黑名单只需在 token 自然过期前存在，到期自动清除，不产生长期垃圾数据；这也意味着登出接口只影响"当前这一枚 token"，多端登录的其他 token 不受影响（本期单端使用，可接受；如需"全端登出"后续可改为按 userId 维护版本号/白名单方案）。
