...
| 后端 | Redis 7 | 登录失败计数限流（INCR + EXPIRE，5 次锁 10 分钟，见接口文档 §2.2 / 需求 S-3） |
...
- **账号**：注册 / 登录 / 个人信息，JWT 鉴权 + 密码 BCrypt，支持改昵称与头像；登录失败限流（Redis，连续失败 5 次锁 10 分钟）
...
- JDK 17+ / Maven 3.8+ / MySQL 8.x（运行中）/ Node.js 18+ / Redis 7.x（登录限流已启用，**后端启动前必须运行**）
...
登录限流（需求 S-3 登录部分）已实现：登录接口强依赖 Redis，未启动会导致登录报错。Windows 推荐 Docker：
